...
/* ── Два варіанти плити ───────────────────────────────────────────
   Денний (без суфікса) — стандартна іконка сайту, та сама зелена плита
   за замовчуванням (data-theme="green"), стопи з --plate-base
   (photo/photo.css). Її не чіпаємо.

   Темний — не «нічна тема сайту», а системний темний Apple: градієнт,
   яким iOS 18 підкладає темні іконки додатків, #2C2C2E → #171719.
   Нейтральний графіт без зеленого відтінку; на 75% проходить рівно
   через #1C1C1E, тобто через systemBackground темного iOS. Стопи
   пройдені рівним кроком по OKLab — тим самим способом, яким пораховані
   стопи зеленої плити: три стопи дають видиме око злам на середньому.

   CSS-кут 154° лишається спільний для обох: плита має читатися тією
   самою конструкцією, міняється тільки її колір.

   grainOpacity темного трохи нижчий за зелений (0.22): зерно тут не
   тільки фактура, а й засіб проти смуг на темному градієнті, але на
   майже чорному воно проступає помітніше, тож підібрано на око. */
const variants = [
  {
  ...
  {
    suffix: '-dark', label: 'dark (iOS system)',
    ink: '#eaf1e7', grainOpacity: 0.17,
    stops: [
      ['#2c2c2e', 0], ['#2a2a2c', 8.3], ['#28282a', 16.7], ['#272729', 25],
      ['#252527', 33.3], ['#232325', 41.7], ['#212123', 50], ['#202022', 58.3],
      ['#1e1e20', 66.7], ['#1c1c1e', 75], ['#1a1a1c', 83.3], ['#19191b', 91.7], ['#171719', 100],
    ],
  },
];
